...
class IndexSampler:
    def __init__(self, general_es, indices):
        self.general_es = general_es
    ...
    def __call__(self):
        ...
        self.counter += 1
        return self.general_es.get_sample_by_index(idx)


class ERA5DaliESDataloader(object):
    def get_pipeline(general_es, batch_size, device_id=0, indices=None):
        sampler = IndexSampler(general_es, indices if indices is not None else list(range(len(general_es))))

        pipe = Pipeline(batch_size=batch_size, num_threads=4, device_id=device_id)
        with pipe:
            data = fn.external_source(source=sampler, num_outputs=4 if general_es.zenith_angle else 2)
            # unpack based on zenith angle usage
            if general_es.zenith_angle:
                inp, tar, zen_inp, zen_tar = data
                # You can do any other transforms here if needed
                pipe.set_outputs(inp, tar, zen_inp, zen_tar)
            else:
                inp, tar = data
                pipe.set_outputs(inp, tar)
